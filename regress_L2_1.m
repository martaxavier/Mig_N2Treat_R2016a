function [B,stats] = regress_L2_1(X,Y,varargin)

%   [B,STATS] = regress_L2_1(X,Y,...) performs L2 and 
%   L1-constrained linear least squares fits - elastic net fit - 
%   relating the predictors in X to the responses in Y. 
%
...
% It is used to terminate the (ever-less penalized) fits 
% when it becomes clear that we are overfitting
...
if isempty(lambda)
    ...
    % Sanity check on user-supplied lambda sequence
...
[B,Intercept,lambda,mse,mseIter,reachedMaxIter] = ...
    regressFit(X,Y,lambda,alpha,standardize,reltolB,reltolMSE,...
...
for i = 1:nLambda
    ...
    for numIter = 1:maxIter
        ...
        [b,active] = cdescentCycle(X0,Y0, ...
            b,active,lam*gamma,alpha,gamma);
        ...
        if norm( (b-bold) ./ (1.0 + abs(bold)), Inf ) < reltolB || ...
            ...
            potentially_active = thresholdScreen(X0,Y0,...
                b,ever_active,lam*gamma*alpha);
            
            if any(potentially_active)
                new_active = active | potentially_active;
                [b,new_active] = cdescentCycle(X0,Y0, ...
                    b,new_active,lam*gamma,alpha,gamma);
            else
                new_active = active;
            end
    ...
    % Halt if we have exceeded a threshold
    % on the percent of residual variance 
    % left unexplained
...
function [b,active] = cdescentCycle(X0, Y0, ...
    b, active, lam, alpha, gamma)
%
...
shrink_factor = ones(1,P) * (1 + lam*(1 - alpha));
threshold = lam*alpha;
...
for j=a
    ...
    % Soft thresholding
    b(j) = sign(bj) .* max((abs(bj) - threshold), 0) ./ shrink_factor(j);
